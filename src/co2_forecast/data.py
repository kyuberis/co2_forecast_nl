...
def make_splits(df, cfg):
    ...
    print(f"  Val:   {len(val_df) - len(train_df)} new rows ({cfg['train_end']} → {cfg['val_end']})")
...
def build_datasets(df, max_train_idx, max_val_idx, min_val_pred_idx, min_test_pred_idx, cfg):
    ...
    train_loader = training.to_dataloader(train=True,   batch_size=cfg["batch_size"],     num_workers=2, pin_memory=True)
    val_loader   = validation.to_dataloader(train=False, batch_size=cfg["batch_size"] * 2, num_workers=2)
